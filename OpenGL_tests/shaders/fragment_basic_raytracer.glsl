...
layout(location = 0) uniform vec3 player;
layout(location = 3) uniform vec2 camera;
layout(location = 5) uniform float circle_angle;
...
{	
	for (int i = 0; i < n_spheres; i++)
	{
		...
		spheres[i] = vec4(300 * cos(a+circle_angle * 0.1), 300.0 * sin(a+circle_angle * 0.1) + 250, 0.0, 100.0);
	}
	
	vec4 final_color = vec4(0.0);
	vec3 view_origin = vec3(player.x, player.y, player.z - 768.0);
	vec2 screen_vec = gl_FragCoord.xy;
	
	vec3 current_vector = vec3(screen_vec.x-512.0, screen_vec.y-384.0, 0.0)-vec3(0.0, 0.0, -768.0);
	
	float sin_a = sin(camera.x);
	float cos_a = cos(camera.x);
	float sin_a_y = sin(-camera.y);
	float cos_a_y = cos(-camera.y);
